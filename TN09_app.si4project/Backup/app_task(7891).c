...
void bleScanCallBack(deviceScanList_s *list)
{
    ...
    {
        if (my_strpach(list->list[i].broadcaseName, "M02"))
        {
...
void tiltSetRequest(void)
{
    sysinfo.doTiltFlag = 1;
}
...
{
    static uint16_t runtick;
    static uint8_t fsm = 0;
    if (sysparam.tiltalm == 0)
    {
        return;
    }
    if (sysinfo.doTiltFlag == 0)
    {
        return;
    }
    switch (fsm)
